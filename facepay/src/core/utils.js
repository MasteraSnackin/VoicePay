...
export async function clearAsyncStorage() {
  try {
    ...
    // Best-effort cleanup for demo resets
  }
}
...
export async function clearEncryptedStorage() {
  ...
  try {
    await AsyncStorage.removeItem("GeneralBackup");
  } catch {
    // Best-effort cleanup for demo resets
  }
}
...
export async function clearAppStorage() {
  await Promise.all([clearAsyncStorage(), clearEncryptedStorage()]);
}
...
export function replaceRoute(router, path) {
...
export function getLocalDemoUsdConversion(tokens) {
  const demoPrices = {
    HBAR: 0.12,
    USDC: 1,
    SAUCE: 0.06,
  ...
  return tokens.map((token) => demoPrices[token.symbol] ?? 0);
}

export function getLocalDemoTrustScore(accountId) {
  ...
  return Math.max(72, Math.min(98, 70 + (numericSeed % 29)));
}
